...
- 前端：Next.ts 最新稳定版 + React + JavaScript。
- 当前确认的 npm latest 版本：`next@16.2.7`，但初始化时仍使用 `@latest`。
- 实时通信：Socket.IO。
- 游戏服务端：Node.ts + Fastify + Socket.IO + JavaScript。
- 参数校验：Zod。
- 数据库：PostgreSQL。
...
不要把实时游戏循环直接塞进 Next.ts API Routes。Next.ts 负责页面和 UI，游戏服务端单独运行，前端通过 Socket.IO 连接游戏服务。

### 2.1 初始化要求

前端使用最新版 Next.js，并明确使用 JavaScript，不使用 TypeScript：

```bash
pnpm create next-app@latest apps/web --js --use-pnpm
```
...
```json
{
  ...
  "private": true,
  "scripts": {
    "dev": "pnpm --parallel --filter @lie/web --filter @lie/game-server dev",
    "dev:web": "pnpm --filter @lie/web dev",
    "dev:server": "pnpm --filter @lie/game-server dev",
    "lint": "pnpm -r lint",
    "test": "pnpm -r test"
  },
...
- `apps/game-server` 负责注册登录、房间、游戏状态、Socket.IO、Redis、PostgreSQL。
- `packages/shared` 只放纯 TavaScript 工具和 Zod schema，不依赖 React、Fastify、Redis 或数据库。
...
`apps/game-server/src/auth/password.ts` 示例：
...
注册、登录和当前用户信息走游戏服务端 REST API，由 `apps/game-server` 提供。
...
const privateGameState = {
  ...
  hands: {
    "player-1": [{ id: "AS", rank: "A", suit: "S" }]
  },
  discardPile: [
    {
      card: { id: "7H", rank: "7", suit: "H" },
      playedByPlayerId: "player-2",
      turnSeq: 3
    }
  ],
  ...
  lastPlay: {
    ...
    declaredRank: "K",
    declaredCount: 1,
    actualCards: [{ id: "7H", rank: "7", suit: "H" }],
    turnSeq: 3
  },
  turnSeq: 4,
  winnerPlayerId: null,
  createdAt: 1780800000000,
  updatedAt: 1780800000000
}
```
...
使用明确的 Socket.IO 事件名，并为每个 payload 编写 Zod schema。不要依赖 TypeScript 类型来保证运行时安全。
...
// room:ready
{ roomId: "uuid", ready: true }

// game:start
{ roomId: "uuid" }

// game:playCards
{ roomId: "uuid", cardIds: ["AS", "AH"] }
...
```js
socket.emit("room:updated", publicGameState)
socket.emit("game:updated", publicGameState)
socket.emit("game:event", publicGameEvent)
socket.emit("game:challengeResolved", challengeResult)
socket.emit("game:finished", { winnerPlayerId, state: publicGameState })
socket.emit("error:message", { code: "ERROR_CODE", message: "错误信息" })
```

### 8.3 公开游戏事件

```js
const cardsPlayedEvent = {
  type: "cards_played",
...
const challengeResolvedEvent = {
  ...
  wasTruthful: false,
  revealedCards: [{ id: "7H", rank: "7", suit: "H" }],
  pileTakenByPlayerId: "player-1",
  turnSeq: 5
}

const playerConnectionEvent = {
...
apps/game-server/src/game/deck.ts
apps/game-server/src/game/engine.ts
apps/game-server/src/game/public-state.ts
```
...
export function playCards(state, actorPlayerId, cardIds) {}
export function challengeLastPlay(state, challengerPlayerId) {}
export function toPublicGameState(state, viewerPlayerId) {}
```
...
- 初始化 `apps/web`，使用 `pnpm create next-app@latest apps/web --js --use-pnpm`。
- 初始化 `apps/game-server`，使用 Fastify + JavaScript。
- 初始化 `packages/shared`。
