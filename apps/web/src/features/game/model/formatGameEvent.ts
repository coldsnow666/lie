/**
 * @Description: 统一把公开游戏事件转换成房间页和对局页共用的中文文案。
 *
...
export function formatGameEvent(event: PublicGameEvent) {
  ...
  if (event.type === "cards_played") {
    return event.playMode === "follow"
      ? `玩家跟 ${event.declaredRank}，打出 ${event.cardCount} 张`
      : `玩家声明 ${event.declaredRank}，打出 ${event.cardCount} 张`;
  }
